...
export default function ReviewsSwiper() {
  return (
    <section className="relative w-full py-16 md:py-20 lg:py-24 bg-[#070c1b]">
      <div className="max-w-[1400px] mx-auto px-4 md:px-6 lg:px-8">
        <div className="mb-16 md:mb-20">
          <div className="flex items-center justify-between mb-8 md:mb-12">
            <button className="swiper-button-prev-reviews hidden md:flex w-14 h-14 rounded-xl bg-[#0b1c4a] border border-white/10 text-white hover:text-[#0047ff] flex items-center justify-center transition-all duration-200 hover:scale-110">
              <FaArrowLeft size={20} />
            </button>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-black font-display text-white text-center flex-1">
              Отзывы наших клиентов
            </h2>
            <button className="swiper-button-next-reviews hidden md:flex w-14 h-14 rounded-xl bg-[#0b1c4a] border border-white/10 text-white hover:text-[#0047ff] flex items-center justify-center transition-all duration-200 hover:scale-110">
          ...
          >
            {reviews.map((item) => (
              <SwiperSlide key={item.id} className="!h-auto">
                <div className="bg-[#0b1c4a] rounded-3xl p-6 md:p-8 h-full flex flex-col border border-white/10 shadow-xl min-h-[280px]">
